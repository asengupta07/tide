...
export default function NewStrategy() {
  ...
  const ship = async () => {
    ...
    try {
      ...
      await run("approve", async () => {
        ...
        if (aw < wethAmt) last = await wait(await writeContractAsync({ address: ADDR.weth, abi: erc20Abi, functionName: "approve", args: [ADDR.aqua, wethAmt] }));
        if (au < usdcAmt) last = await wait(await writeContractAsync({ address: ADDR.usdc, abi: erc20Abi, functionName: "approve", args: [ADDR.aqua, usdcAmt] }));
        return last;
      });
  ...
  const amountOk = (v: string, dec: number) => /^\d+(\.\d+)?$/.test(v) && Number(v) > 0 && (v.split(".")[1]?.length ?? 0) <= dec;
  const canShip = isConnected && (avail?.available || !!strat) && amountOk(weth, 18) && amountOk(usdc, 6) && !busy;
          ...
          <Bezel>
            <div className="space-y-8 p-6 md:p-8">
              ...
              <Field label={`λ, inventory exposed per block: ${lambda / 100}%`} hint="lower cuts arbitrage loss, raises drift. The frontier suggests 50% at 60% volatility.">
                <input type="range" aria-label="visibility per block, percent" aria-valuetext={`${lambda / 100} percent`} min={500} max={10000} step={100} value={lambda} onChange={(e) => setLambda(Number(e.target.value))} className="w-full" />
              </Field>
              <div className="grid gap-5 sm:grid-cols-3">
                <Field label={`Fee: ${fee / 100}%`} hint="on every trade; it is what backs the deep curve">
                  <input type="range" aria-label="fee, percent" aria-valuetext={`${fee / 100} percent`} min={1} max={100} step={1} value={fee} onChange={(e) => { const f = Number(e.target.value); setFee(f); setDelta((d) => Math.min(d, maxDelta(n, f))); }} className="w-full" />
                </Field>
                <Field label={`N, virtual depth: ${n}×`} hint="follow-on trades see a pool N times deeper">
                  <input type="range" aria-label="virtual depth multiplier" aria-valuetext={`${n} times`} min={1} max={16} step={1} value={n} onChange={(e) => { const v = Number(e.target.value); setN(v); setDelta((d) => Math.min(d, maxDelta(v, fee))); }} className="w-full" />
                </Field>
                <Field label={`δ, drift bound: ${delta / 100}%`} hint={`max price move the deep curve honours; the fee backs up to ${maxDelta(n, fee) / 100}% at ${n}×`}>
                  <input type="range" aria-label="drift band, percent" aria-valuetext={`${delta / 100} percent`} min={1} max={Math.max(1, maxDelta(n, fee))} step={1} value={delta} onChange={(e) => setDelta(Number(e.target.value))} className="w-full" />
                </Field>
              </div>

              <div className="grid gap-5 sm:grid-cols-2">
                <Field label="WETH inventory" hint="wrapped from ETH if short">
                  <input aria-label="WETH inventory" inputMode="decimal" value={weth} onChange={(e) => setWeth(e.target.value.trim())} className={`num w-full rounded-xl border bg-white/[0.03] px-4 py-3 outline-none focus:border-accent ${weth && !amountOk(weth, 18) ? "border-bad/50" : "border-white/10"}`} />
                </Field>
                <Field label="USDC inventory" hint="minted if short">
                  <input aria-label="USDC inventory" inputMode="decimal" value={usdc} onChange={(e) => setUsdc(e.target.value.trim())} className={`num w-full rounded-xl border bg-white/[0.03] px-4 py-3 outline-none focus:border-accent ${usdc && !amountOk(usdc, 6) ? "border-bad/50" : "border-white/10"}`} />
                </Field>
              </div>
          ...
          <Bezel small>
            <ol className="space-y-3 p-5">
              {[
                ...
                ["params", "Set parameters", "TideApp.init from your wallet; guardrails start at their defaults"],
