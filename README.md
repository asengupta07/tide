...
Owners can publish or unpublish a live strategy or a reusable template from the sharing panel on its dashboard. `/app/explore` lists only explicitly published entries. `/trade` introduces the trader venue with a live market preview; `/trade/market` is the compact execution terminal. Shared live strategies deep-link to the terminal, whose liquidity list independently discovers every initialized, funded Tide strategy from the registry because that liquidity is already public and executable on-chain. Empty, incomplete and test-only entries stay hidden. For each order, the trade API quotes all executable Tide LPs for the pair in one multicall and selects the best single-strategy execution (highest output for exact-in, lowest input for exact-out); a connected maker's own strategies are excluded. It does not yet split an order across LPs or route through external venues. Templates alone never create a live market. Templates save parameters and manager guardrails as a snapshot; they do not follow later strategy changes. Publishing a new snapshot updates the template explicitly. Unpublishing prevents future Explore discovery and template retrieval, but cannot hide on-chain liquidity or recall copies already loaded or created.
